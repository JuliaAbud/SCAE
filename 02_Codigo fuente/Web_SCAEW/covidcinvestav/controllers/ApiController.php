...
class ApiController extends Controller
{
	...
	public function actionConcurrenciavisita()//Obtiene la concurrencia del establecimiento donde se encuentra el usuario actualmente
    {
		Yii::$app->response->format = Response::FORMAT_JSON;
        $model = new Individuo();
		$modelLogin = new LoginForm();
		...
		$model->load(Yii::$app->request->post());
		if($modelLogin->login()){
			$negocio=$this->findVisitaactual($model->codigo)->negocio;
			if($negocio->idnegocio>0)
				return $negocio->concurrencia>=$negocio->aforo;
			return false;
		}
		return $modelLogin->getErrors();
    }
	public function actionHistorialnegocio()
    {
		Yii::$app->response->format = Response::FORMAT_JSON;
        $model = new Negocio();
		$modelLogin = new LoginForm();
		...
		if($modelLogin->login()){
			$model->load(Yii::$app->request->post());
			return $this->findVisitasnegocio($model->idnegocio);
		}
		return $modelLogin->getErrors();
    }
	...
	protected function findModelNegocioid($id)
    {
	...
	function findVisitaactual($codigo)
    {
		$tablevisitas= new visitas;
       $query = " SELECT visita.idvisita,codigoindividuo,
	   negocio.idnegocio,nombre,fechavisita 
				 FROM visita 
				 INNER JOIN negocio ON visita.idnegocio=negocio.idnegocio
				 where 
				visita.codigoindividuo='$codigo' 
				and now() between visita.fechavisita 
				and  DATE_ADD(visita.fechavisita, INTERVAL negocio.tiempopermanencia MINUTE)
				order by fechavisita desc limit 1";
	...
	protected function findVisitasnegocio($id)
    {
		...
		where idnegocio=$id and DATE(visita.fechavisita)=date(now())  
	...
	/*delete FROM visita
WHERE DATE(fechavisita)< DATE_ADD(NOW(), INTERVAL -15 day)*/
}
